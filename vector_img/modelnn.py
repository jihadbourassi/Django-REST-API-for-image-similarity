...
#    def getSimilarityForNew(self,image,image_name):
#        """
#        Just functionized your code for new images.
#        Saves the json into img folder
#        """
#        named_nearest_neighbors = []
#        nearest_neighbors = self.model.get_nns_by_vector(image, self.n_neigh)
#        for j in nearest_neighbors :
#            neighbor_file_name = self.file_index_to_file_name[j]
#            neighbor_file_vector = self.model.get_item_vector(j)

#            similarity = 1 - spatial.distance.cosine(image, neighbor_file_vector)
#            rounded_similarity = int((similarity * 10000)) / 10000.0
#            if (image_name != neighbor_file_name) and (rounded_similarity > 0.90):
...
#                })
            ...
            #    named_nearest_neighbors = []
            #    named_nearest_neighbors.append({
            #    'message': "no similar image above 0.9 similarity"
            #    })  
#        if  named_nearest_neighbors != [] :
#            print(image_name)
#            with open('app/img/'+str(image_name)+'.json', 'w') as out:
...
#                json.dump(named_nearest_neighbors, out)
    ...
    #def readVectorized(self):
    #        """
    #        Load the vectorized form, get the similarities
    #        """
    #    vector = np.loadtxt(str(self.path+".npz"))
    #    name = os.path.basename(self.path).split('.')[0]
    #        self.getSimilarityForNew(vector,name)
