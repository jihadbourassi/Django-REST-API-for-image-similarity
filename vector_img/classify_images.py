...
BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),'tmp\imagenet')
  ...
  def __init__(self,
    ...
    if not label_lookup_path:
      label_lookup_path = os.path.join(
          BASE_DIR, 'imagenet_2012_challenge_label_map_proto.pbtxt')
    if not uid_lookup_path:
      uid_lookup_path = os.path.join(
          BASE_DIR, 'imagenet_synset_to_human_label_map.txt')
...
def create_graph():
  ...
  with tf.gfile.FastGFile(os.path.join(
      BASE_DIR, 'classify_image_graph_def.pb'), 'rb') as f:
